...
async def stream_answer(
    ...
    try:
        ...
        # Off subject. The refusal is written here rather than generated, so
        # declining costs nothing and always says the same thing -- a model
        # asked to refuse will sometimes negotiate instead.
        yield {"type": "token", "text": guards.OFF_TOPIC_REFUSAL}
        yield {"type": "refused", "reason": _tripwire_reason(tripped)}
        yield {"type": "done"}
        return
